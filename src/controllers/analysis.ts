...
const analyzeProject = async (req: Request, res: Response) => {
  ...
  for (const file of sourceFiles) {
    ...
    for(let attempt = 1; attempt <= MAX_RETRIES; attempt++){
      try {
        console.log(
          `AI Review : ${file.path} | attempt ${attempt}/${MAX_RETRIES}`
        );
        ...
        lastError = error;
        console.log(
          `AI Review failed : ${file.path} | attempt : ${attempt}/${MAX_RETRIES}`
        )
    ...
    if(review){
      ...
      console.error(`AI review permanantly failed for file ${file.path}:`,
        lastError
      );
      ...
      aiReviews.push({
        ...
        status : "failed",
        error : 
        lastError instanceof Error ? lastError.message : "AI review failed"
      })
    }
  }
  ...
  await prisma.aiReview.create({
    data: {
      ...
      issues: {
        create: aiReview.review.issues.map((issue: any) => ({
...
const getProjectAnalysis = async (req: Request, res: Response) => {
